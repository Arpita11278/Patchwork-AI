...
class PatchworkAgent:
    ...
    def analyze_with_llm(self, content: str, file_path: str):
        ...
        prompt = (
            ...
            "Step 1 — Look for REAL correctness issues: typos in variable/identifier names, undefined variables, "
            "off-by-one errors, wrong operators, missing return statements, logic errors, iteration over modifying "
            "sets, missing dictionary keys (KeyErrors), unhandled exceptions, or security issues. Only report a "
            "Step 1 finding if you are CONFIDENT it will actually cause wrong behavior or a crash with the code as "
            "written — not something that merely *could* go wrong under a hypothetical future change.\n\n"
            "Step 2 — ONLY if Step 1 found nothing, check for quality/style gaps (e.g. missing docstrings, missing "
            "type hints, misleading complexity comments). Skip Step 2 entirely if you already listed a Step 1 issue.\n\n"
            "HARD RULES:\n"
            "- List each finding as one bullet point (starting with '- '), one short sentence, no hedging.\n"
            "- Never include a finding you go on to describe as 'not a bug', 'not strictly wrong', 'safe', 'no "
            "immediate issue', or similar — if it isn't actually wrong, don't mention it at all.\n"
            "- Never assume, quote, or refer to a comment, docstring, or context not literally visible in this "
            "snippet.\n"
            "- Report at most 4 bullet points total — pick the most important ones.\n\n"
            "Reply with exactly the word 'NONE' if there are no confident correctness issues and no quality/style gaps.\n\n"
            f"Code:\n{snippet}"
        )
