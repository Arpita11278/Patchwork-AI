...
class PatchworkAgent:
    ...
    def process_github_repo(self, repo_url: str):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if changes_made:
                    self.create_pull_request(temp_dir, repo_url)
            else:
    ...
    def create_pull_request(self, repo_path: str, repo_url: str):
        if not self.github_token:
            print("\n⚠️ GITHUB_TOKEN not found in .env. Skipping Pull Request creation.")
            print("💡 Tip: Add GITHUB_TOKEN to automatically create PRs for approved fixes.")
            return
    ...
    def analyze_with_llm(self, content: str, file_path: str):
        ...
        prompt = f"Analyze this code from '{file_path}' for bugs, vulnerabilities, or code smells. If there is a clear issue, reply with a short 1-sentence description of the issue. If the code is perfectly fine and needs no changes, reply with exactly the word 'NONE'.\n\nCode:\n{content[:3000]}"
    ...
    def fix_with_llm(self, content: str, issue_description: str):
        ...
        # Updated prompt to completely avoid AI-like talk and perfectly preserve style
        ...
        data = {
            "model": self.model,
            "messages": [{"role": "user", "content": prompt}],
            "temperature": 0.0 # Using 0.0 for maximum consistency and lack of creativity
        }
        try:
            response = requests.post(self.api_url, headers=headers, json=data, timeout=15)
            if response.status_code == 200:
